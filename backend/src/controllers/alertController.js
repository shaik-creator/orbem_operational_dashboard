...
const ALERT_TYPES_BY_ROLE = {
  [ROLES.ADMIN]: null,
  [ROLES.OPERATIONS]: ['Delayed Shipments', 'Pending Documents'],
  [ROLES.ACCOUNTS]: ['Overdue Payments'],
  [ROLES.WAREHOUSE]: ['Delayed Shipments', 'Pending Documents']
};

function alertFilterForUser(user) {
  const allowedTypes = ALERT_TYPES_BY_ROLE[normalizeRole(user?.role)];
  if (!allowedTypes) return { sql: '', params: [] };
  return {
    sql: `WHERE type IN (${allowedTypes.map(() => '?').join(', ')})`,
    params: allowedTypes
  };
}
...
const listAlerts = asyncHandler(async (req, res) => {
  const filter = alertFilterForUser(req.user);
...
const getUnreadCount = asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `SELECT COUNT(*) AS count FROM alerts ${filter.sql ? `${filter.sql} AND` : 'WHERE'} is_read = 0`,
    filter.params
  );
...
const markAsRead = asyncHandler(async (req, res) => {
  ...
  const allowed = await query(
    `SELECT id FROM alerts ${filter.sql ? `${filter.sql} AND` : 'WHERE'} id = ? LIMIT 1`,
