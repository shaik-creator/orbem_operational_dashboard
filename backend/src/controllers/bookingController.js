...
function appendRoleFilters(filters, user) {
  const role = normalizeRole(user?.role);
  if (role === ROLES.OPERATIONS) {
    return {
      sql: filters.sql
        ? `${filters.sql} AND (b.assigned_owner_id = ? OR b.owner_id = ? OR (b.assigned_owner_id IS NULL AND b.owner_id IS NULL))`
        : 'WHERE (b.assigned_owner_id = ? OR b.owner_id = ? OR (b.assigned_owner_id IS NULL AND b.owner_id IS NULL))',
      params: [...filters.params, user.id, user.id]
    };
  }
  return filters;
}

function stripFinancialFields(row, user) {
  const role = normalizeRole(user?.role);
  if ([ROLES.ADMIN, ROLES.ACCOUNTS].includes(role)) return row;
  const { quotation_amount, invoice_amount, paid_amount, balance_amount, payment_date, payment_method, due_date, payment_status, ...safeRow } = row;
  return safeRow;
}

function canAccessBookingRecord(booking, user) {
  const role = normalizeRole(user?.role);
  if (role !== ROLES.OPERATIONS) return true;
  if (!booking.assigned_owner_id && !booking.owner_id) return true;
  return Number(booking.assigned_owner_id) === Number(user.id) || Number(booking.owner_id) === Number(user.id);
}
...
const getBooking = asyncHandler(async (req, res) => {
  ...
  if (!canAccessBookingRecord(rows[0], req.user)) {
    throw createHttpError('Access denied', 403);
  }

  const booking = stripFinancialFields(rows[0], req.user);
